...
func (c *Client) storeCache(transport adapter.DNSTransport, question dns.Question, message *dns.Msg, timeToLive uint32) {
	...
	if c.dnsCache != nil {
		transportTag := ""
		if c.independentCache && transport != nil {
			transportTag = transport.Tag()
		}
		_ = c.dnsCache.SaveDNSCache(
			transportTag,
			question.Name,
			question.Qtype,
			question.Qclass,
			message.Copy(),
			time.Now().Add(time.Second*time.Duration(timeToLive)),
		)
		return
	}
	if c.disableExpire {
...
func (c *Client) loadResponse(question dns.Question, transport adapter.DNSTransport) (*dns.Msg, int) {
	if c.dnsCache != nil {
		...
		response, expiresAt, loaded := c.dnsCache.LoadDNSCache(transportTag, question.Name, question.Qtype, question.Qclass)
		if !loaded {
			return nil, 0
		}
		timeNow := time.Now()
		if timeNow.After(expiresAt) {
			_ = c.dnsCache.DeleteDNSCache(transportTag, question.Name, question.Qtype, question.Qclass)
			return nil, 0
		}
		nowTTL := int(expiresAt.Sub(timeNow).Seconds())
		if nowTTL < 0 {
			nowTTL = 0
		}
		for _, recordList := range [][]dns.RR{response.Answer, response.Ns, response.Extra} {
