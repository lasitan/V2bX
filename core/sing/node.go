...
func (b *Sing) AddNode(tag string, info *panel.NodeInfo, config *conf.Options) error {
	...
	if info.Type == "vless" {
		o, ok := c.Options.(*option.VLESSInboundOptions)
		if !ok {
			return fmt.Errorf("unexpected inbound options type for vless")
		}
		if o.Transport != nil && o.Transport.Type == "ws" {
			// Only support plain websocket on shared port.
			if o.TLS != nil && o.TLS.Enabled {
				needWsMux = false
			} else {
				needWsMux = true
			}
			wsListenIP = config.ListenIP
			wsPort = uint16(info.Common.ServerPort)

			// determine route using transport options
			wsHost = ""
			wsPath = "/"
			if o.Transport.WebsocketOptions.Path != "" {
				wsPath = o.Transport.WebsocketOptions.Path
			}
			if o.Transport.WebsocketOptions.Headers != nil {
				if h, ok := o.Transport.WebsocketOptions.Headers["Host"]; ok {
					if len(h) > 0 {
						wsHost = h[0]
					}
				}
			...
			if wsHost == "" {
				// fallback to panel common host if transport header not present
				wsHost = info.Common.Host
			}
		}
		if needWsMux {
			if b.wsMux == nil {
				return fmt.Errorf("ws mux manager not initialized")
			}
			_, err = b.wsMux.Ensure(wsListenIP, wsPort)
			if err != nil {
				return fmt.Errorf("start ws mux listener error: %s", err)
			}

			loopbackIP, err := loopbackFor(wsListenIP)
			if err != nil {
				return fmt.Errorf("determine loopback ip error: %s", err)
			}
			backendPort, err := allocatePort(loopbackIP)
			if err != nil {
				return fmt.Errorf("allocate backend port error: %s", err)
			}
			wsBackend = net.JoinHostPort(loopbackIP, fmt.Sprint(backendPort))

			// override inbound to bind internal loopback port instead of external shared port
			addr, err := netip.ParseAddr(loopbackIP)
			if err != nil {
				return fmt.Errorf("parse loopback ip error: %s", err)
			}
			o.Listen = (*badoption.Addr)(&addr)
			o.ListenPort = backendPort

			// normalize ws path from URL if needed
			if u, e := url.Parse(wsPath); e == nil {
				if u.Path != "" {
					wsPath = u.Path
				}
			}
	...
	if needWsMux {
		if err := b.wsMux.Register(tag, wsListenIP, wsPort, wsHost, wsPath, wsBackend); err != nil {
			_ = in.Remove(tag)
			return err
		}
	}
